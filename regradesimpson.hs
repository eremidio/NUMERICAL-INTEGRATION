{-
VAMOS ESCREVER UM PROGRAMA EM HASKELL QUE CALCULA INTEGRAIS DEFINIDAS USANDO A REGRA DE SIMPSON
...
factor_array n  = map (\i-> factor i n)[0..(2*n)]
...
value_array n x a = map (\i -> a+fromInteger(i)*x)[0..(2*n)] -- n é o index do termo na lista, x é o tamanho do subintervalo e a o limite inferior do intervalo
...
main = do
 ...
 --Passo de integração
 let step = (b-a)/(fromInteger(n)*2.0)
 ...
 --Exibindo o resultado
 putStrLn("O valor da integral definida é "++show((foldr(+) 0 array4)*step/3)++".")
